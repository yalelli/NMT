...
namespace NMT.Nators
{
    class CSmarAct
    {
        ...
        ******************/
        ...
        public const uint SA_INVALID_COMPONENT_ERROR = 154;
        public const uint SA_PERMISSION_DENIED_ERROR = 157;
        ...
        [DllImport(@"MCSControl.dll", CallingConvention = CallingConvention.Cdecl)] // Positioner
        public static extern uint SA_GetChannelProperty_S(uint systemIndex, uint channelIndex, uint key, ref int value);

        [DllImport(@"MCSControl.dll", CallingConvention = CallingConvention.Cdecl)] // Positioner
        public static extern uint SA_SetChannelProperty_S(uint systemIndex, uint channelIndex, uint key, int value);

        [DllImport(@"MCSControl.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern uint SA_EPK(uint selector, uint subSelector, uint property);

        [DllImport(@"MCSControl.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern uint SA_GetClosedLoopMoveAcceleration_S(uint systemIndex, uint channelIndex, ref uint acceleration);

        [DllImport(@"MCSControl.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern uint SA_SetClosedLoopMoveAcceleration_S(uint systemIndex, uint channelIndex, uint acceleration);

        //channel properties
        //component
        public const uint SA_GENERAL = 1;

        //sub component
        public const uint SA_LOW_VIBRATION = 2;

        //property
        public const uint SA_OPERATION_MODE = 1;
        
        //valid value range
        public const int SA_DISABLED = 0;
        public const int SA_ENABLED = 1;
    }
}
